...
export default function WeeklyPlannerPage() {
  ...
  const filtered = useMemo(
    () =>
      plannerItems.filter(
        (item) =>
          (week === 'All' || formatDate(item.weekCommencing) === week) &&
          (department === 'All' || item.department === department) &&
          (activityType === 'All' || item.activityType === activityType) &&
          (status === 'All' || item.status === status),
      ),
    [week, department, activityType, status, plannerItems],
  );
  ...
  const weeks = Array.from(
    new Set(plannerItems.map((item) => formatDate(item.weekCommencing))),
  );
  const departments = Array.from(
    new Set(plannerItems.map((item) => item.department)),
  );
  const activityTypes = Array.from(
    new Set(plannerItems.map((item) => item.activityType)),
  );
  const statuses = Array.from(
    new Set(plannerItems.map((item) => item.status)),
  );
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="text-slate-500">
              <tr>
                <th className="pb-3 text-left">Week</th>
            ...
            <tbody>
              {filtered.map((item) => (
                <tr key={item.id} className="border-t border-slate-100">
                  <td className="py-3">
                    {formatDate(item.weekCommencing)}
                  </td>
                  <td className="py-3">{item.traineeName}</td>
                  <td className="py-3">
                    {item.activityType} · {item.process}
                  </td>
